...
public class RecipeActivityTest {
    ...
    public ActivityTestRule<RecipeActivity> activityTestRule = new ActivityTestRule<RecipeActivity>(RecipeActivity.class) {
        ...
        protected Intent getActivityIntent() {
            ...
            intent.putExtra(RecipeActivity.EXTRA_ID, 1);
            return intent;
        }
    };

    @Test
    public void testStepListVisibility() {
        onView(withId(R.id.container_main)).check(matches(isDisplayed()));
        onView(withId(R.id.list)).check(matches(isDisplayed()));
    }


    // check step without video

    @Test
    public void testStepListAction1() {
        ...
        onView(withId(R.id.pb_loading_video)).check(matches(Matchers.not((isDisplayed()))));
    }

    // check step with video

    @Test
    public void testStepListAction2() {
        onView(withId(R.id.list)).perform(RecyclerViewActions.actionOnItemAtPosition(2, click()));
        onView(withId(R.id.pb_loading_video)).check(matches(isDisplayed()));
    }
